...
async def welcome(Authorize: AuthJWT=Depends()):
    ...
    return {'message':"This is SignUp Page"}
...
async def refresh_token(Autorize: AuthJWT=Depends()):
    ...
    try:
        access_lifetime = datetime.timedelta(minutes=30)
        refresh_lifetime = datetime.timedelta(days=3)
        Autorize.jwt_refresh_token_required() # valid access tokenni talab qiladi
        current_user = Autorize.get_jwt_subject() # access tokendan usernameni ajratib oladi

        # Databasedan userni filterlash orqali topamiz
        ...
        new_access_token = Autorize.create_access_token(subject=db_user.username,expires_time=access_lifetime)
        response_model = {
            "success": True,
            "code": 200,
