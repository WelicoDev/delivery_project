...
async def product_create(product:ProductModel,Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN , detail="Only admin can add product")
...
async def list_all_products(Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN , detail="Only admin can see all products")
...
async def get_product_by_id(id: int , Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        if product:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND , detail=f"Product with {id} product is not found")
...
async def delete_product_by_id(id: int , Authorize: AuthJWT = Depends()):
    ...
    if current_user.is_staff:
        ...
        if product:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND , detail="Product with ID {id} is not found")
